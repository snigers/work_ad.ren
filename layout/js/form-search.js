...
(function (global){
  ...
  ym.modules.define('map-view', [
    ...
    var MapView = defineClass(function () {
      ...
      function onPolygonLoad (json, coords) {
        ...
        ymaps.route([[55.753215, 37.622504], coords]).then(
          function (res) {
            ...
            map.geoObjects.add(myRoute = res);
            ...
            if (!objectsOutMoscow._objects.length) {
              // Внутри МКАД

              intersectCoords = false;
      ...
      function getMkadDistance(map, intersectCoords, coords) {
        ...
        if (intersectCoords) {
          ...
          multiRoute.model.events.add("requestsuccess", function (event) {
            ...
            var mkadDistance = multiRoute.getRoutes().get(0).properties.get("distance").value;

            $("#mkadDistance").html(Math.ceil(mkadDistance/1000));

            $(".cart-address-distance").show();

            $("#cart_shipping_type_2").attr("checked","checked").trigger("change");
          ...
          $(".cart-address-distance").hide();

          $("#cart_shipping_type_1").attr("checked","checked").trigger("change");
      ...
      $(".search-query").on("blur change", function () {
